## -------------------------------------------------
##
## Script name: Compute 1-LOD boundaries for parametric recessive linkage
##
## Purpose: For each chromosome with a significant recessive HLOD
...
pl_out <- fread(here("data", "linkage", "pl_rec.out"))
...
# chromosomes with a significant recessive HLOD peak
for (chr in c(2, 16, 20)) {
...
fwrite(pl_1lod_boundaries, 
       here("data", "genomic_ranges", "pl_rec_1lod_regions_of_interest.csv"))
