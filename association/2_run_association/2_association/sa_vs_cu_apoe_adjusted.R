## -------------------------------------------------
##
## Script name: Single-variant association: superagers vs. cognitively unimpaired (APOE-adjusted)
##
...
##   1-LOD linkage intervals, with APOE e4 carrier status included as an
##   additional covariate to assess associations independent of APOE genotype.
##
## -------------------------------------------------
##
## Notes:
...
invisible(lapply(gds_files[roi$chr],
                 function(x) run.assoc(x, out_prefix = "assoc.sa.v.cu", subdir = "sa_v_cu_revision", window = TRUE)))
