## -------------------------------------------------
##
## Script name: Single-variant association: superagers vs. Alzheimer's disease (APOE-adjusted)
##
## Purpose: Run single-variant association tests comparing superagers
##   (SA) to individuals with Alzheimer's disease (AD) within 1-LOD linkage
##   intervals, with APOE e4 carrier status included as an additional covariate
##   to assess associations independent of APOE genotype.
##
## -------------------------------------------------
##
## Notes:
...
# join PCs to pheno; derive binary APOE e4 carrier variable
pheno_pcs <- pheno %>%
  mutate(sample.id = as.character(sample.id),
         E4_CARRIER = ifelse(E4_DOSAGE > 0, 1, 0)) %>%
...
# logistic mixed model: age, sex, APOE e4 carrier status, and top 2 PCs as covariates;
...
invisible(lapply(gds_files[roi$chr],
                 function(x) run.assoc(x, out_prefix = "assoc.sa.v.ad", subdir = "sa_v_ad_revision", window = TRUE)))
